...
const Project = ({
	...
	return (
		<div
			id={id}
			className={classNames('project-card', isHighlighted && 'highlight')}
		>
			<div className='flex'>
	...
	return (
		<>
			...
			<div className='mw-30r'>
				...
				<p>
					I use <ELi href='https://codeberg.org/athena'>Codeberg</ELi>{' '}
					for personal projects, and{' '}
					<ELi href='https://github.com/xplato'>GitHub</ELi> for more
					"professional" projects—most often, code for my business,
					Infinium.
				</p>
			</div>
			...
			<div className='w-100p grid s-grid-2 gap-2r portrait-grid-1 my-2r'>
				{projects.map((project) => (
					<Project
						key={project.title}
						{...project}
						isHighlighted={url.get('h')?.trim() === project.id}
					/>
				))}
			</div>
		</>
