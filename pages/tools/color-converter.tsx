...
const ColorConverter = () => {
	...
	useEffect(() => {
		...
		if (mode === 'RGBToHex' && value) {
			...
			setResult(RGBToHex(v[0], v[1], v[2]));
		}
	}, [value, mode]);
				...
				<div className='container flex-c'>
					<div className='ui radius-8 border-ui-2 shadow-xs'>
						<div className='w-100p flex-sb px-1-5r py-1r border-bottom-ui-2'>
							...
							<Button.Action
								className='hy-button icon-only'
								onClick={refreshRandomColor}
						...
						<div className='p-1-5r flex-c text-c'>
							{result ? (
								<div className='flex align-c justify-c flex-row'>
									<Button.Action onClick={onResultClick}>
										Copy — {result}
									</Button.Action>
									<i className='j-icon ml-0-5r'>
										{copyStatus === 'success' && (
											<CheckIcon />
										)}
										{copyStatus === 'fail' && <XIcon />}
									</i>
								</div>
							) : (
								<span className='caption-text sm-ls'>
									Random: {getColor()}
								</span>
							)}
						</div>
